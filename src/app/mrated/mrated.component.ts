...
@Component({
  selector: 'app-mrated',
  templateUrl: './mrated.component.html',
  styleUrls: ['./mrated.component.css']
})
export class MratedComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.myMS.getratedMovies(this.currentPage).subscribe({
      next: (response) => {
  ...
  searchMovies(movieName: string) {
    this.myMS.searchratedMovies(movieName, this.currentPage).subscribe({
      next: (data) => {
  ...
  changeLanguage() {
    ...
    this.myMS.getratedMovies().subscribe({
      next: (response) => {
        this.allMovies = response.results;
      },
    });
